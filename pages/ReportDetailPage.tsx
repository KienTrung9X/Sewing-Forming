...
const ReportDetailPage: React.FC = () => {
  ...
  const [attachedFiles, setAttachedFiles] = useState<Array<{file: File; preview?: string; isImage: boolean}>>([]);
  const [uploadedAttachmentLinks, setUploadedAttachmentLinks] = useState<string[]>([]);
      ...
      <div className="mt-8 border-t border-gray-200 dark:border-slate-700 pt-6">
        ...
        {!isEditing ? (
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className={labelClasses}>Đính kèm (Ảnh, .xls, .xlsx)</label>
              <input
                type="file"
              ...
              {attachedFiles.length > 0 && (
                <div className="mt-3 grid grid-cols-3 gap-3">
                  {attachedFiles.map((a, idx) => (
                    <div key={idx} className="border p-2 rounded-md flex flex-col items-center">
                      {a.isImage && a.preview ? (
                        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                        // @ts-ignore
                        <img src={a.preview} alt={a.file.name} className="h-20 w-20 object-cover rounded-md mb-2" />
                      ) : (
                        <div className="h-20 w-20 flex items-center justify-center bg-gray-100 dark:bg-slate-700 rounded-md mb-2 text-xs text-center p-1">{a.file.name}</div>
                      )}
                      <div className="text-xs text-gray-600 dark:text-gray-300 text-center break-words">{a.file.name}</div>
                      <button type="button" onClick={() => setAttachedFiles(prev => prev.filter((_, i) => i !== idx))} className="mt-2 text-red-600 text-xs">Xóa</button>
                    </div>
                  ))}
                </div>
              )}
